...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.PATCHRIGHT_ENABLED': {
    title: 'Patchright 搜索服务',
    summary: '启用基于本地浏览器的 Patchright 独立搜索服务（抓取百度/夸克/360）。',
    usage: '需要先安装 patchright 并手动启动服务（run-patchright-server.bat），再开启本开关。',
    valueNotes: ['开启后作为 SearXNG 之后的兜底搜索引擎。'],
    impact: ['影响无 API Key 场景下的新闻搜索覆盖能力；服务未启动时该引擎自动降级，不影响其他引擎。'],
    notes: ['浏览器生命周期与主进程完全独立，主进程崩溃或超时不影响搜索服务进程。'],
  },
  'settings.data_source.PATCHRIGHT_BASE_URL': {
    title: 'Patchright 服务地址',
    summary: 'Patchright 独立搜索服务的基础地址。',
    usage: '默认 http://127.0.0.1:8931；仅在开启 Patchright 搜索服务时使用。',
    valueNotes: ['支持远程部署时修改为其他地址。'],
    impact: ['服务不可达时该引擎自动 failover 到下一个引擎。'],
    notes: ['建议使用 http/https 协议。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.PATCHRIGHT_ENABLED': {
    ...
    summary: 'Enables the local-browser Patchright standalone search service (Baidu/Quark/360).',
    usage: 'Install patchright and start the service (run-patchright-server.bat) before enabling this switch.',
    valueNotes: ['Acts as a fallback search engine after SearXNG.'],
    impact: ['Broadens news search coverage without API keys; a stopped service degrades gracefully to other engines.'],
    notes: ['The browser lifecycle is fully independent from the main process.'],
  },
  'settings.data_source.PATCHRIGHT_BASE_URL': {
    title: 'Patchright Base URL',
    summary: 'Base URL of the Patchright standalone search service.',
    usage: 'Defaults to http://127.0.0.1:8931; only used when Patchright is enabled.',
    valueNotes: ['Can point to a remote deployment.'],
    impact: ['An unreachable service falls back to the next search engine.'],
    notes: ['Use http/https URLs.'],
  },
  'settings.data_source.ENABLE_CHIP_DISTRIBUTION': {
